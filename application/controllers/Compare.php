...
// ประชาสัมพันธ์
    public function cg_base() {
       
        $data['rs'] = $this->My_model->get_all_order('tb_std_score', 'id ASC');

        $this->load->view("layout/header");
        $this->load->view("compare/cg_base", $data);
            ...
            foreach ($allDataInSheet as $arr) {
                ...
                if ($i != 0) {
                    foreach (array_keys($arr) as $key => $value) {
                        $insertdata[$j][$colName[$value]] = $arr[$value];
                    }
                    $j++;
                }
                $i++;
            }
            $result = $this->import->importdata($table, $insertdata);
            if ($result) {
