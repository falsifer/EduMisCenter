...
Class Course_detail extends CI_Controller {
    ...
    public function dc_base_setting() {
        ...
        $this->load->view("course_detail/dc_standard_score_base", $data);
    ...
    public function course_by_classid_term_edyear() {
        ...
        if ($courselist) {
            foreach ($courselist as $r) {
                ...
                $course_detail = $this->My_model->get_where_row('tb_course_detail', array(
                    'tb_course_id' => $r['course_id'],
                    'tb_human_resources_01_id' => $this->session->userdata('hr_id'),
                        )
                );

                $count = count($course_detail);
                if ($count > 0) {
                    $output .= "&nbsp;<button type=\"button\" class=\"btn btn-success btn-result\" id=\"" . $course_detail['id'] . "\" onclick=\"ClearThisCourseDetail(this)\"><i class=\"icon-ok icon-large\"></i> เลือกแล้ว</button>";
                } else {
                    $output .= "&nbsp;<button type=\"button\" class=\"btn btn-secondary btn-result\" id=\"" . $r['course_id'] . "\" onclick=\"InsertCourseDetail(this)\">คลิ๊กเพื่อเลือก</button>";
                }


                //       $output .= "&nbsp;<button type=\"button\" class=\"btn btn-secondary btn-result\" id=\"" . $r['course_id'] . "\" onclick=\"InsertCourseDetail(this)\"><i class=\"icon-save icon-large\"></i> คลิ๊กเพื่อเลือก</button>";
...
//                $output .= "&nbsp;<button type=\"button\" class=\"btn btn-danger \" id=\"" . $r['course_id'] . "\" onclick=\"DeleteThisCourse(this)\"><i class=\"icon-trash icon-large\"></i> ลบ</button>";
    ...
    function clear_course_detail() {
        $id = $_POST['id'];

        $this->My_model->delete_data('tb_ed_schedule', array('tb_course_detail_id' => $id));

        $this->My_model->delete_data('tb_course_detail', array('id' => $id));
    }

    function insert_course_detail() {
        $cid = $_POST['id'];
        $class = $_POST['class'];
        $term = $_POST['term'];
        $arr = array(
        ...
        $cd = $this->My_model->get_where_row('tb_course_detail', $arr);

        if (!isset($cd['id'])) {
            $cdid = $this->My_model->insert_data('tb_course_detail', $arr);
            if (isset($cdid)) {

                $sec = $this->My_model->get_where_row('tb_ed_section', array('tb_ed_section_department' => $this->session->userdata('department'), 'tb_ed_section_class_sub' => rand(1, 5)));
                $room = $this->My_model->get_where_row('tb_ed_room', array('tb_ed_school_register_class_id' => $class, 'tb_classroom_room' => 1));


                if (isset($sec) && isset($room)) {
                    $arrSC = array(
                        'tb_ed_schedule_term'=>$term,
                        'tb_ed_section_id' => $sec['id'],
                        'tb_ed_room_id' => $room['id'],
                        'tb_course_detail_id' => $cdid,
                        'tb_ed_schedule_day' => rand(1, 5),
                        'tb_ed_schedule_department' => $this->session->userdata('department')
                    );
                    $this->My_model->insert_data('tb_ed_schedule', $arrSC);
                }
            }
        }
    }
