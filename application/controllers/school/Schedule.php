...
//        $this->My_model->delete_data('tb_course_detail', array('tb_course_id' => $id));
        $default = $this->My_model->get_where_order('tb_course_detail', array('tb_course_id' => $id), 'tb_human_resources_01_id');
        $darr = array();
        foreach ($default as $r) {
            if(in_array($r['tb_human_resources_01_id'],$hrid)){
                array_push($darr, $r['tb_human_resources_01_id']);
            }else{
                $this->My_model->delete_data('tb_course_detail', array('tb_human_resources_01_id' => $r['tb_human_resources_01_id']));
            }
        }
        foreach ($hrid as $hr) {
            if (!in_array($hr, $darr)) {
        ...
        if (sizeof($tmp) > 1) {
            ...
            for ($i = 1; $i <= $day; $i++) {
                ...
                for ($j = 1; $j <= sizeof($sec); $j++) {
                    print("Y:".$yearly." C:".$tt." L:".$lev." R:".$rid." T:".$term." S:".$sec[$j - 1]." D:".$i);
...
//                    $timetable = $this->My_model->get_where_row('tb_ed_schedule', array('tb_ed_section_id' => $sec[$j-1], 'tb_ed_room_id' => $rid,'tb_ed_schedule_day'=>$i));
                    ...
                    if (isset($timetable['tb_course_detail_id'])) {
