...
";
        ...
        \">" . $row['tb_icare_sdq_topic'] . "</td>";
            ...
            <button type=\"button\" class=\"btn btn-warning btn-sdq-edit\" id=\"" . $row['id'] . "\"><i class=\"icon-pencil icon-large\"></i> แก้ไข</button>
            <button type=\"button\" class=\"btn btn-danger btn-sdq-delete\" id=\"" . $row['id'] . "\"><i class=\"icon-trash icon-large\"></i> ลบ</button>
            </td>";
            }
            $output .= "</tr> ";
        }
...
//                SELECT tb_student_base_id,tb_sdq_type,sum(tb_std_icare_sdq_score_score) FROM `tb_std_icare_sdq_score` a inner join tb_icare_sdq b on a.tb_icare_sdq_id = b.id inner join tb_icare_sdq_type c on b.tb_icare_sdq_type = c.id inner join tb_student_base d on a.tb_student_base_id = d.id where a.tb_std_icare_sdq_score_assessor = 'Teacher' and a.tb_std_icare_sdq_score_term = 1 and a.tb_std_icare_sdq_score_edyear = 2562 group by tb_student_base_id,tb_sdq_type
                ...
                if ($chk) {
                    $output .= "&nbsp;<button type='button' class='btn btn-warning' onclick='ParentSDQ(this)' id='" . $Std['StdId'] . "' ><i class='icon-print icon-large'></i> ผู้ปกครอง</button>";
                } else {
                ...
                $chk = $this->Icare_model->checkCompleteSDQ($Std['StdId'], 'Student', $Term, $EdYear);
                if ($chk) {
                    $output .= "&nbsp;<button type='button' class='btn btn-success' onclick='StudentSDQ(this)' id='" . $Std['StdId'] . "' ><i class='icon-print icon-large'></i> นักเรียน</button>";
                } else {
...
//        $Assessor = "Teacher";
        ...
        }elseif($Assessor=="Parent"){
        $output .= "<div style='margin: 30px 0px;line-height: 35px;font-size: 20px;font-weight: bold;'>แบบผู้ปกครองประเมินนักเรียน (SDQ)</div>";
        }elseif($Assessor=="Student"){
        $output .= "<div style='margin: 30px 0px;line-height: 35px;font-size: 20px;font-weight: bold;'>แบบประเมินตนเอง (SDQ)</div>";
        }
        $output .= "</center>";
...
//        $output .= datethaifull($Student['std_birthday']);
        ...
        $output .= "<input type='hidden' id='Assessor' value='".$Assessor."' />";

        $output .= "<table class='table table-hover table-bordered display' style='width:100%'>";
        $output .= "<thead>";
        $output .= "<tr>";
        ...
        foreach ($SDQType as $rType) {
            ...
            foreach ($SDQArray as $rSDQ) {
                ...
                if($Assessor=="Student"){
                    $output .= "<td style='width: 45%;text-align: left;'>ฉัน" . $rSDQ['tb_icare_sdq_topic'] . "</td>";
                }else{
...
//                $output .= "<input value='" . $Assessor . "/" . $StdId . "/" . $rType['id'] . "' />";
                    ...
                    if (isset($rRecord['tb_std_icare_sdq_score_score'])) {
                        ...
                        switch ($rRecord['tb_std_icare_sdq_score_score']) {
                            case 2:
                                $output .= "<td class='TdSelect' style='text-align:center' id='R," . $rRecord['id'] . ",2' onclick='SelectThisTd(this)'><i class='icon-ok icon-large'></i></td>";
                                $output .= "<td class='TdSelect' style='text-align:center' id='R," . $rRecord['id'] . ",1' onclick='SelectThisTd(this)'></td>";
                                $output .= "<td class='TdSelect' style='text-align:center' id='R," . $rRecord['id'] . ",0' onclick='SelectThisTd(this)'></td>";
                                break;
                            case 1:
                                $output .= "<td class='TdSelect' style='text-align:center' id='R," . $rRecord['id'] . ",2' onclick='SelectThisTd(this)'></td>";
                                $output .= "<td class='TdSelect' style='text-align:center' id='R," . $rRecord['id'] . ",1' onclick='SelectThisTd(this)'><i class='icon-ok icon-large'></i></td>";
                                $output .= "<td class='TdSelect' style='text-align:center' id='R," . $rRecord['id'] . ",0' onclick='SelectThisTd(this)'></td>";
                                break;
                            case 0:
                                $output .= "<td class='TdSelect' style='text-align:center' id='R," . $rRecord['id'] . ",2' onclick='SelectThisTd(this)'></td>";
                                $output .= "<td class='TdSelect' style='text-align:center' id='R," . $rRecord['id'] . ",1' onclick='SelectThisTd(this)'></td>";
                                $output .= "<td class='TdSelect' style='text-align:center' id='R," . $rRecord['id'] . ",0' onclick='SelectThisTd(this)'><i class='icon-ok icon-large'></i></td>";
                                break;
                        }
                    } else {
                        $output .= "<td style='background:grey;' style='text-align:center' class='TdSelect' id='F," . $rSDQ['id'] . ",2' onclick='SelectThisTd(this)'></td>";
                        $output .= "<td style='background:grey;' style='text-align:center' class='TdSelect' id='F," . $rSDQ['id'] . ",1' onclick='SelectThisTd(this)'></td>";
                        $output .= "<td style='background:grey;' style='text-align:center' class='TdSelect' id='F," . $rSDQ['id'] . ",0' onclick='SelectThisTd(this)'></td>";
                    }
                } else {
...
//                $output .= "<input value='" . $Assessor . "/" . $StdId . "/" . $rType['id'] . "' />";
    ...
    public function student_insert_sdq_score() {
        ...
        $chk = $this->My_model->get_where_row('tb_std_icare_sdq_score',$arr);
        if(isset($chk['id']))
        {
            $Status = "R";
        }        
