...
Class SISImport extends CI_Controller {
...
//                        $comment = $objPHPExcel->getActiveSheet()->getComment($this->getColumnEng($i) . '1')->getText();
//                        if ($comment != "บิดา" && $comment != "มารดา" && $comment != "ผู้ปกครอง") {
                            if ($col_name != "" && $col_name != 'id' && $arr[$value] != "") {
...
//                    print_r($arry);
                    ...
                    if (isset($id)) {
                        ...
                        //ข้อมูลเด็กเข้าห้องเรียน

                        $this->db->select('*,c.tb_student_base_id as stdid')->from('tb_ed_classroom c');
                        $this->db->join('tb_ed_room r', 'c.tb_ed_room_id = r.id');
                        $this->db->join('tb_ed_school_register_class rc', 'rc.id = r.tb_ed_school_register_class_id');
//                        $this->db->join('tb_student_base std','std.id = c.tb_student_base_id');
                        $this->db->where(array('c.tb_student_base_id' => $id, 'rc.tb_ed_school_register_class_edyear' => $this->input->post('inEdYear')));
//                        $this->db->where(array('std.std_code' => $stdcode,'rc.tb_ed_school_register_class_edyear'=> $this->input->post('inEdYear')));
                        $stdRS = $this->db->get()->row_array();
                        if ($stdRS['stdid'] == null) {
                            $std_arr = array(
                                "tb_ed_classroom_number" => $i,
                                "tb_student_base_id" => $id,
                                "tb_ed_room_id" => $this->input->post('inStdClassRoomM')
                            );
                            $this->My_model->insert_data('tb_ed_classroom', $std_arr);
                        }
...
//                $op .= "<td style=\"text-align:center;\">" . $row->tb_ed_school_class_name . " " . $row->tb_ed_school_class_level . "</td>";
    ...
    function delete_all_std() {
        $rid = $_POST['rid'];


        //get std_id list
        $crs = $this->My_model->get_where_order('tb_ed_classroom', array('tb_ed_room_id' => $rid), 'tb_student_base_id');
...
//        print($std);
        //del tb_std_address
        $this->My_model->delete_data('tb_std_address', 'std_id in (' . $std . ')');
        //del tb_std_family
        //get tb_outsider list
        $frs = $this->My_model->get_where_order('tb_std_family', 'std_id in (' . $std . ')', 'tb_outsider_id');
        $tb_outsider_id = "";

        $i = 1;
        foreach ($frs as $r) {
            if ($i != count($frs)) {
                $tb_outsider_id .= $r['tb_outsider_id'] . ",";
            } else {
                $tb_outsider_id .= $r['tb_outsider_id'];
            }
            $i++;
...
//        print($frs);
        ...
        $this->My_model->delete_data('tb_std_family', 'std_id in (' . $std . ')');

        //del tb_std_whn
        $this->My_model->delete_data('tb_std_wnh', 'std_id in (' . $std . ')');

        //del tb_register_course
        $this->My_model->delete_data('tb_register_course', 'tb_student_base_id in (' . $std . ')');
        //del tb_ed_activity_score
        $this->My_model->delete_data('tb_ed_activity_score', 'tb_student_id in (' . $std . ')');
        //del tb_ed_capacity_score
        $this->My_model->delete_data('tb_ed_capacity_score', 'tb_student_id in (' . $std . ')');
        //del tb_ed_character_score
        $this->My_model->delete_data('tb_ed_character_score', 'tb_student_id in (' . $std . ')');
        //del tb_ed_rw_analysis_score
        $this->My_model->delete_data('tb_ed_rw_analysis_score', 'tb_student_id in (' . $std . ')');
        //del tb_school_administrator_score
        $this->My_model->delete_data('tb_school_administrator_score', 'tb_student_base_id in (' . $std . ')');
        //del tb_std_course_score
        $this->My_model->delete_data('tb_std_course_score', 'tb_student_base_id in (' . $std . ')');
        //del tb_std_course_final_score
        $this->My_model->delete_data('tb_std_course_final_score', 'tb_student_base_id in (' . $std . ')');
        //del tb_std_course_midterm_score
        $this->My_model->delete_data('tb_std_course_midterm_score', 'tb_student_base_id in (' . $std . ')');
        //del tb_std_icare_sdq_score
        $this->My_model->delete_data('tb_std_icare_sdq_score', 'tb_student_base_id in (' . $std . ')');
        //del tb_std_midterm_score
        $this->My_model->delete_data('tb_std_midterm_score', 'tb_student_base_id in (' . $std . ')');
        //del tb_std_before_register
        $this->My_model->delete_data('tb_std_before_register', 'tb_student_base_id in (' . $std . ')');
        //del tb_std_picture ลบรูปใน upload/sch_id/std/std_id
        $this->My_model->delete_data('tb_std_picture', 'own_id in (' . $std . ')');
        //del tb_std_absent_record
        $this->My_model->delete_data('tb_std_absent_record', 'tb_student_base_id in (' . $std . ')');

        //del tb_member
        foreach ($crs as $r) {
            $stdRs = $this->My_model->get_where_row('tb_student_base', array('id' => $r['tb_student_base_id']));
            if (isset($stdRs['tb_member_id']))
                $this->My_model->delete_data('tb_member', array('id' => $stdRs['tb_member_id']));
        }
        foreach ($frs as $r) {
            $stdRs = $this->My_model->get_where_row('tb_outsider', array('id' => $r['tb_outsider_id']));
            if (isset($stdRs['tb_member_id']))
                $this->My_model->delete_data('tb_member', array('id' => $stdRs['tb_member_id']));
        }
        if ($tb_outsider_id != "") {
            $this->My_model->delete_data('tb_fm_career', 'tb_outsider_id in (' . $tb_outsider_id . ')');
            $this->My_model->delete_data('tb_outsider', 'id in (' . $tb_outsider_id . ')');
        }

        //del tb_ed_classroom
        $this->My_model->delete_data('tb_ed_classroom', 'tb_student_base_id in (' . $std . ')');
        //del tb_student_base
        $this->My_model->delete_data('tb_student_base', 'id in (' . $std . ')');
    }

    function delete_std_by_id_list() {
        ...
        //del tb_std_picture ลบรูปใน upload/sch_id/std/std_id
        foreach ($student_list as $r) {
            $sch_id = $this->Std_model->get_std_base_w_stdid_return_row($r);
            unlink('upload/' . $sch_id . '/std/' . $r);
        }
        $this->My_model->delete_data('tb_std_picture', 'own_id in (' . $std . ')');
        //del tb_std_absent_record
        $this->My_model->delete_data('tb_std_absent_record', 'tb_student_base_id in (' . $std . ')');
