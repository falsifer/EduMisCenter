...
//        $data['rClass'] = $this->My_model->get_where_order('tb_group_learning', array('id !=' => '0'), 'id asc');
//        $data['rClass'] = $this->My_model->get_where_order('tb_ed_school_class', array('id !=' => '0'), 'id asc');
    ...
    public function ms_insert() {
        if ($_FILES['inMdFile']['name'] != "") {
            ...
            $this->upload->do_upload("inMdFile");
        ...
        if($this->input->post('inMdType') !='FILE'){
            $link = $this->input->post('inMdLink');
        }else{
            $link = $filename;
        }
        $arr = array(
//            "tb_course_id" => $this->input->post('inSubject'),
    ...
    public function ms_delete(){
        $id = $_POST['id'];
        $row = $this->My_model->get_where_row("tb_media", array('id' => $id)); // ดึงข้อมูลจากตารางหลัก tb_km_base;
        // ลบข้อมูลภาพประกอบ
        @unlink('upload/' . $row['tb_media_link']);
        $this->My_model->delete_data('tb_media', array('id' => $id)); // ลบข้อมูลจาก tb_km_history
    }
//========== end ==========//  
//-------------- Media online ----------//
...
//        $output .= "</div>";
    ...
    public function blog_base_all_blog() {
        ...
        $this->db->order_by('tb_blog_createdate desc');
...
//            $output .= " </button>";
            ...
            $output .= "<div class=\"row\" onmousedown='return false;' onselectstart='return false;'> " . substr($r->tb_blog_content, 0, 200) . "<button type='button' class='btn btn-link' onclick='BlogDetail(this)' id='" . $r->id . "'> ...อ่านเพิ่มเติม</button>";
