...
//////////////ภาพปิด///////////
        $arr = array(
            ...
            "std_birth_hospital" => $this->input->post('inStdBirthhospital'),
            "std_bloodtype" => $this->input->post('inStdBloodtype'),
            "std_birthday" => $this->input->post('inStdBirthyear') . "-" . insert_zero_f_position($this->input->post('inStdBirthmonth'), 2) . "-" . insert_zero_f_position($this->input->post('inStdBirthday'), 2)
        );
        if ($id != "") {
            $this->My_model->update_data('tb_student_base', array('id' => $id), $arr);
        }
        ...
        if ($id != "") {
            $this->My_model->update_data('tb_std_address', array('id' => $addid), $arr);
        }


//ข้อมูลด้านผู้ปกครอง
        $arr = array(
            "tb_outsider_titlename" => $this->input->post('inFmTitlename'),
            "tb_outsider_firstname" => $this->input->post('inFmFirstname'),
            "tb_outsider_lastname" => $this->input->post('inFmLastname'),
            "tb_outsider_idcard" => $this->input->post('inFmIdcard'),
            "tb_outsider_religion" => $this->input->post('inFmReligion'),
            "tb_outsider_nationality" => $this->input->post('inFmNationality'),
            "tb_outsider_ethnicity" => $this->input->post('inFmEthnicity'),
            "tb_outsider_about" => $this->input->post('inFmAbout'),
            "tb_outsider_status" => $this->input->post('inFmStatus'),
            "tb_outsider_relationship" => $this->input->post('inFmRelationship')
        );
        if ($id != "") {
            $this->My_model->update_data('tb_outsider', array('id' => $did), $arr);
        }
