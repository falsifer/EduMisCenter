...
//
    ...
    public function hr_give_up() {
        ...
        $this->load->view("layout/header");
        $this->load->view(($this->session->userdata('department') == 'กองการศึกษา') ? "hr_give_up/index_zone" : "hr_give_up/index", $data);
        $this->load->view("layout/footer");
    }

    // การส่งเสริมยกย่องเชิดชูเกียรติฯ
    public function get_give_up_filter() {
...
//        
        ...
        if ($this->input->post('school') != '') {
            $ar = array('hr_department' => $this->input->post('school'));
            $arr = array_merge($arr, $ar);
        } else {
            $ar = array('hr_department' => $this->session->userdata('department'));
            $arr = array_merge($arr, $ar);
        }

        if ($this->input->post('gl') != '') {
            $ar = array('hr_group_learning' => $this->input->post('gl'));
            $arr = array_merge($arr, $ar);
        }


        $rs = $this->My_model->join2table_where_result('tb_human_resources_01 a', 'tb_hr_give_up b', 'b.hr_id = a.id', $arr, 'hr_thai_name asc');
        $out = "";

        $row = 1;
        foreach ($rs as $r) {
            $out .= "<tr>";
            $out .= "<td style=\"text-align:center;\">" . $row . "</td>";
            $out .= "<td>" . shortdate($r['give_up_date']) . "</td>";
            $out .= "<td>" . $r['give_up_topic'] . "</td>";
            $out .= "<td>" . $r['hr_thai_symbol'] . $r['hr_thai_name'] . $r['hr_thai_lastname'] . "</td>";
            $out .= "<td>" . $r['give_up_office'] . "</td>";
            if (file_exists('upload/' . $r['give_up_document']) && !empty($r['give_up_document'])) {
                $out .= anchor(base_url() . 'upload/' . $r['give_up_document'], 'เอกสาร', array('rel' => 'lytebox'));
            } else {
                $out .= "<td>&nbsp;</td>";
            }
            if (file_exists('upload/' . $r['give_up_image1']) && !empty($r['give_up_image1'])) {
                $out .= anchor(base_url() . 'upload/' . $r['give_up_image1'], 'เอกสาร', array('rel' => 'lytebox'));
            } else {
                $out .= "<td>&nbsp;</td>";
            }
            if (file_exists('upload/' . $r['give_up_image2']) && !empty($r['give_up_image2'])) {
                $out .= anchor(base_url() . 'upload/' . $r['give_up_image2'], 'เอกสาร', array('rel' => 'lytebox'));
            } else {
                $out .= "<td>&nbsp;</td>";
            }
            if (file_exists('upload/' . $r['give_up_image3']) && !empty($r['give_up_image3'])) {
                $out .= anchor(base_url() . 'upload/' . $r['give_up_image3'], 'เอกสาร', array('rel' => 'lytebox'));
            } else {
                $out .= "<td>&nbsp;</td>";
            }
            $out .= "<td>" . $r['give_up_comment'] . "</td>";
            $out .= "<td>" . $r['give_up_department'] . "</td>";
            if (trim($r['give_up_department']) == trim($this->session->userdata('department'))) {
                $out .= "<td style=\"text-align:center;\">
                                    <button type=\"button\" class=\"btn btn-warning btn-edit\" id=\"{$r['id']}\"><i class=\"icon-pencil icon-large\"></i> แก้ไข</button>
                                    <button type=\"button\" class=\"btn btn-danger btn-delete\" id=\"{$r['id']}\"><i class=\"icon-trash icon-large\"></i> ลบ</button>
                                </td>";
            } else {
                $out .= "<td>&nbsp;</td>";
            }
