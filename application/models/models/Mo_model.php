...
class Mo_model extends CI_Model {
    ...
    function media_online_base() {
        $this->db->select("a.*, b.*, c.*, a.id AS aid")->from("tb_course a");
        $this->db->join("tb_subject b", "b.id = a.tb_subject_id");
        $this->db->join("tb_ed_classroom_online c", "c.tb_course_id = a.id");
        $this->db->order_by("a.id asc");
        $rs = $this->db->get();
        if ($rs->num_rows() > 0) {
            return $rs->result_array();
