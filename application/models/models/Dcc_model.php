...
class Dcc_model extends CI_Model {
    ...
    function get_course_purpose_by_unitid($UnitId) {
        $this->db->select("*")->from("tb_course_purpose");
...
//            $this->db->where('f.tb_human_resources_01_id', $this->session->userdata('hr_id'));
    ...
    function fetch_member($param) {
        ...
        $this->db->where('tb_subject_department', $this->session->userdata('department'));
...
//$this->db->where("tb_lesson_plan_id", $id);
    ...
    function gen_subject_code($SId, $CId) {
        ...
        if ($MyClassQ[0]['tb_ed_school_class_level'] < 3) {
            $str3 = $MyClassQ[0]['tb_ed_school_class_level'];
        } else {
            $str3 = $MyClassQ[0]['tb_ed_school_class_level'] - 3;
        }
    ...
    function course_by_class_term_edyear($classid, $term, $edyear) {

        $this->db->select("*, a.id as course_id , c.id as gl_id ")->from("tb_course a");
        $this->db->join("tb_subject b", "b.id = a.tb_subject_id");
        $this->db->join("tb_group_learning c", "c.id = b.tb_group_learning_id");
        $this->db->join("tb_ed_school_register_class d", "d.id = a.tb_ed_school_register_class_id");
        $this->db->join("tb_ed_school_class e", "e.id = d.tb_ed_school_class_id");
        ...
        if ($term != "") {
            $this->db->where('a.tb_course_term', $term);
//            $this->db->or_where('a.tb_course_term', "0");
        }


        $this->db->order_by("c.id,b.tb_subject_type asc");
        $MyQ = $this->db->get()->result_array();
        if (count($MyQ) > 0) {
            return $MyQ;
        } else {
    ...
    function course_by_class_term_edyear_hr_id($classid, $term, $edyear) {
    ...
    function get_course_by_id($id) {
        ...
        $this->db->join("tb_ed_school_register_class d", "d.id = a.tb_ed_school_register_class_id");
        $this->db->join("tb_ed_school_class e", "e.id = d.tb_ed_school_class_id");
        $this->db->join("tb_course_detail f", "f.tb_course_id = a.id","LEFT OUTER");
        $this->db->where('a.id', $id);
 ...
 if (count($MyQ) > 0) {
    ...
    function get_kpi_by_unit_id($unit_id) {
        $this->db->select("*,c.id as kpi_id, CONCAT (b.tb_standard_learning_code,\" \",a.tb_kpi_standard_learning_level,\"/\",a.tb_kpi_standard_learning_seq,\" \",tb_kpi_standard_learning_content) as Kpi")->from("tb_kpi_standard_learning a");
        $this->db->join("tb_standard_learning b", "b.id = a.tb_standard_learning_id");
        $this->db->join("tb_kpi_score c", "c.tb_kpi_standard_learning_id = a.id", "left outer");
        $this->db->where("c.tb_unit_learning_id", $unit_id);
        $MyQ = $this->db->get()->result_array();
        if (count($MyQ) > 0) {
            return $MyQ;
        } else {
    ...
    function get_unit_learning_by_id($unit_id) {
        $this->db->select("*, c.tb_group_learning_id")->from("tb_unit_learning a");
        $this->db->join("tb_course b", "b.id = a.tb_course_id");
        $this->db->join("tb_subject c", "c.id = b.tb_subject_id");
        $this->db->join("tb_ed_school_register_class d", "d.id = b.tb_ed_school_register_class_id");
        $this->db->join("tb_ed_school_class e", "e.id = d.tb_ed_school_class_id");
        $this->db->where("a.id", $unit_id);
        $MyQ = $this->db->get()->result_array();
        if (count($MyQ) > 0) {
            return $MyQ;
        } else {
    ...
    function get_kpi_by_class_id_and_grouplearning_id($schoolclass_id, $grouplearning_id) {
        $this->db->select("a.*, b.*, c.*, d.* , c.id as id,a.id as aid")->from("tb_kpi_standard_learning a");
        $this->db->join("tb_standard_learning b", "b.id = a.tb_standard_learning_id");
        $this->db->join("tb_kpi_score c", "c.tb_kpi_standard_learning_id = a.id", "left outer");
        $this->db->join("tb_group_learning_item d", "d.id = b.tb_group_learning_item_id");
        $this->db->where("a.tb_ed_school_class_id", $schoolclass_id);
        $this->db->where("d.tb_group_learning_id", $grouplearning_id);
        $this->db->order_by("a.id asc");
        $MyQ = $this->db->get()->result_array();
        if (count($MyQ) > 0) {
            return $MyQ;
