...
//        $MyQ = $query->result_array();
        ...
        $output .= "<button type=\"button\" class=\"btn btn-primary btn-register-insert-modal\" onclick='InsertWaitingStudent(this)'><i class=\"icon-plus icon-large\"></i> เพิ่มข้อมูล</button>";
        $output .= "&nbsp;&nbsp;";
...
//            $output .= "<button type=\"button\" class=\"btn btn-light btn-show\"><i class=\"icon-user icon-large\"></i>&nbsp;&nbsp;ไม่มีรายชื่อนักเรียนในระบบ</button>";
    ...
    function get_std_registered_list($rid, $cid) {
        ...
        $Count = count($query);
        if ($Count > 0) {
            $this->db->select("a.*,a.id as ed_classroom_id,CONCAT (b.std_titlename,b.std_firstname,\" \",b.std_lastname) as fullname");
            ...
            $MyStdQ = $this->db->get();
            $MyStdQaa = $MyStdQ->result_array();
            $MyStdRoom = count($MyStdQaa);
            ...
            $room_list = $this->db->select("*")->from("tb_ed_room")->where("tb_ed_school_register_class_id", $cid)->order_by("tb_classroom_room asc")->get()->result_array();
...
//                $output .= "&nbsp;<button type=\"button\" class=\"btn btn-warning btn-edit\" onclick=\"StdEdit(this)\" id=\"" . $row->StdId . "\"><i class=\"icon-pencil icon-large\"></i> แก้ไข</button>";
                ...
                $room_output = "<select onchange='RoomListOnChange(this)' id='{$row->StdId}' name='{$row->ed_classroom_id}' class='form-control'>";
                foreach ($room_list as $rList) {
                    if ($rid == $rList['id']) {
                        $room_output .= "<option value='{$rList['id']}' selected>{$rList['tb_classroom_room']}</option>";
                    } else {
                        $room_output .= "<option value='{$rList['id']}'>{$rList['tb_classroom_room']}</option>";
                    }
                }
                $room_output .= "</select>";

                $output .= $room_output;
//                $output .= "&nbsp;<button type=\"button\" class=\"btn btn-danger btn-delete\" onclick=\"StdMoveOut(this)\" id=\"" . $row->StdId . "\"><i class=\"icon-trash icon-large\"></i> ลบ</button>";
...
//                $output .= "<button type=\"button\" class=\"btn btn-danger btn-abc\" id=\"" . $row->id . "\"><i class=\"icon-trash icon-large\"></i> ลบ</button>&nbsp;";
    ...
    function get_std_base_w_schoolid_edyear_return_array($SchoolId, $Edyear) {
        $this->db->select("CONCAT (a.std_titlename,a.std_firstname,\" \",a.std_lastname) as std_fullname,a.id as StdId,a.*");
        ...
        $this->db->join("tb_ed_plan e", "e.id = c.tb_ed_plan_id");
        $this->db->join("tb_ed_school_class f", "f.id = d.tb_ed_school_class_id");
        $this->db->join("tb_school g", "g.id = d.tb_school_id");
//        $this->db->join("tb_std_picture g", "a.id = g.own_id","left outer");
//        if ($SchoolId != null && $SchoolId != "") {
//            $this->db->where_in("d.tb_school_id", $SchoolId);
//        }
//        if ($SchoolId != null && $SchoolId != "") {
        $this->db->where("d.tb_school_id", $SchoolId);
//        }

        $this->db->where("d.tb_ed_school_register_class_edyear", $Edyear);
        $this->db->where("a.tb_student_base_status !=", "W");
...
//            print_r($arr);
            ...
            foreach ($arr as $r) {
                ...
                if ($r['tb_student_base_status'] == "S") {
                    $Status = "<font color='green'>กำลังศึกษา</font>";
                } elseif ($r['tb_student_base_status'] == "G") {
                    $Status = "<font color='blue'>จบการศึกษา</font>";
                }

                $Std_age = datethaitoage($r['std_birthday']);
                ...
                $StdArray = array(
                    ...
                    'std_status' => $Status,
                    'std_age' => $Std_age,
                    'std_school_name' => $r['sc_thai_name'],
                );
...
//                base_url() . std_path($Std['StdId'], $this->session->userdata('sch_id')) . $Std['StdId']
    ...
    function get_std_base_w_stdid_return_row($StdId) {
        ...
        $this->db->select("d.tb_ed_school_register_class_edyear as std_edyear,");
        $this->db->select("e.tb_ed_plan as std_plan,");
        $this->db->select("CONCAT (f.tb_ed_school_class_name,'ชั้นปีที่ ',f.tb_ed_school_class_level) as std_classname,d.tb_school_id as school_id");
        $this->db->from("tb_student_base a");
...
//        $this->db->join("tb_std_health c", "c.own_id = a.id", "left outer");
    ...
    function get_outsider_by_stdid_about_return_row($StdId, $About) {
        $this->db->select("*");
        $this->db->select("CONCAT (c.tb_outsider_titlename,c.tb_outsider_firstname,\" \",c.tb_outsider_lastname) as outsider_fullname");
        $this->db->from("tb_student_base a");
        $this->db->join("tb_std_family b", "b.std_id = a.id");
        $this->db->join("tb_outsider c", "c.id = b.tb_outsider_id");
        $this->db->where("a.id", $StdId);
        $this->db->where("c.tb_outsider_about", $About);
    ...
    function get_std_base_join_visithome_by_roomid_return_array($RoomId) {
        $this->db->select("CONCAT (a.std_titlename,a.std_firstname,\" \",a.std_lastname) as std_fullname,a.id as StdId,a.*");
        $this->db->select("b.tb_ed_classroom_number as std_number,b.id as tb_classroom_id");
        $this->db->select("c.tb_classroom_room as std_room_number,");
        $this->db->select("d.tb_ed_school_register_class_edyear as std_edyear,");
...
//                base_url() . std_path($Std['StdId'], $this->session->userdata('sch_id')) . $Std['StdId']
    ...
    function get_std_base_joinvh_w_roomid_return_array($RoomId) {
        $this->db->select("CONCAT (a.std_titlename,a.std_firstname,\" \",a.std_lastname) as std_fullname,a.id as StdId,a.*");
        $this->db->select("b.tb_ed_classroom_number as std_number,b.id as tb_classroom_id");
        $this->db->select("c.tb_classroom_room as std_room_number,");
        $this->db->select("d.tb_ed_school_register_class_edyear as std_edyear,");
        ...
        $this->db->join("tb_ed_plan e", "e.id = c.tb_ed_plan_id");
        $this->db->join("tb_ed_school_class f", "f.id = d.tb_ed_school_class_id");
        $this->db->join("tb_visit_home_calendar g", "g.tb_student_base_id = a.id");

//        $this->db->join("tb_std_picture g", "a.id = g.own_id","left outer");


        $this->db->where("b.tb_ed_room_id", $RoomId);
        $this->db->order_by("g.tb_visit_home_calendar_date desc");
//        $this->db->where("g.id in","(select max() from tb_std_picture where )");
...
//                base_url() . std_path($Std['StdId'], $this->session->userdata('sch_id')) . $Std['StdId']
    ...
    function get_absent_day_by_student_id($student_id, $startdate, $enddate) {
    ...
    function insert_student_school_bus($hr_id, $sch_id, $std_id, $status) {
        $MyQ = $this->db->select_max("id")->from("tb_vehicle")->where("hr_id", $hr_id)->row_array();
        $arr = array(
        ...
        $this->db->insert('tb_school_bus_transfer', $arr);
    }

    function get_std_base_w_courseid_roomid_return_array($CourseId, $RoomId = null) {

//Check Course
        $this->db->select("a.tb_subject_type as type")->from("tb_subject a");
        $this->db->join("tb_course b", "b.tb_subject_id = a.id");
        $this->db->where("b.id", $CourseId);
        $MyCourseQ = $this->db->get()->row_array();
...
//Student 
        ...
        $this->db->select("e.tb_ed_plan as std_plan,");
        $this->db->select("CONCAT (f.tb_ed_school_class_name,'ชั้นปีที ',f.tb_ed_school_class_level) as std_classname,d.tb_school_id as school_id");
        $this->db->select("g.id as course_register_id,h.id as course_id");
        $this->db->from("tb_student_base a");
        ...
        $this->db->join("tb_register_course g", "g.tb_student_base_id = a.id");
        $this->db->join("tb_course h", "h.id = g.tb_course_id");

//        $this->db->join("tb_std_picture g", "a.id = g.own_id","left outer");

        $this->db->where("h.id", $CourseId);
        if ($MyCourseQ['type'] == "พื้นฐาน") {
            $this->db->where("b.tb_ed_room_id", $RoomId);
        }
