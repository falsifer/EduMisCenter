...
//        $this->db->where('c.id', $plan_id);
    ...
    function get_supervision_by_school($learning_group,$school) {

        $arr = array();
        $this->db->select("c.*")->from("tb_hr_position a");
        ...
        $this->db->where("c.hr_department", $school);     
        $this->db->where_in('tb_hr_position_tier','1,2');
        $this->db->order_by("a.tb_hr_position_tier ASC");
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            $arr = array_merge($arr,$query->result_array());
        }
        ...
        $this->db->join("tb_human_resources_01 c", "c.id = b.tb_hr_id", "left outer");
        $this->db->where("c.hr_department", $school);     
        $this->db->where('hr_group_learning',$learning_group);
        $this->db->where('hr_group_learning_class','หัวหน้า');
        $this->db->order_by("a.tb_hr_position_tier ASC");
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            $arr = array_merge($arr,$query->result_array());
