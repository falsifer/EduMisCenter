...
//------------------------------------------------------------------------------
$mpdf->SetHTMLHeader("<div class='header header-top'>ข้อมูลบุคลากร ประวัติการสร้างผลงาน</div>");
...
$content .= ""
        ...
        . "<th>ที่</th>"
        . "<th>วัน/เดือน/ปี</th>"
        . "<th>เรื่องของผลงาน</th>"
        . "<th>รายละเอียด (โดยสังเขป)</th>"
        . "</tr>"
        . "</thead>"
        . "<tbody>";
$row = 1;
foreach ($rs as $r) {
    $content .= "<tr>"
            ...
            . "<td>{$r['hr09_day']} ". month_num($r['hr09_month'])."&nbsp;&nbsp;{$r['hr09_year']}</td>"
            . "<td>{$r['hr09_topic']}</td>"
            . "<td style='width:45%;'>{$r['hr09_detail']}</td>"
            . "</tr>";
    $row++;
}
$content .= "</tbody></table>";
