<div class="col-md-10 col-md-offset-1">
    <div class="panel panel-primary">
        <div class="panel-heading">## ชนิดแหล่งเรียนรู้ ##</div>
        <ul class="breadcrumb">
...
<script>
    ...
    $(".btn-insert").click(function () {
        ...
        $("h3.modal-title").text("บันทึกข้อมูลชนิดของแหล่งเรียนรู้");
        $("#km-type-modal").modal("show");
    });
    // update data;
    $('#example').on('click', '.btn-edit', function () {
        var uid = $(this).attr('id');
        $.ajax({
            url: '<?php echo site_url('update-km-type'); ?>',
            method: 'post',
            data: {id: uid},
            dataType: 'json',
            success: function (data) {
                ...
                //
                $('h3.modal-title').text('ปรับปรุงข้อมูลชนิดแหล่งเรียนรู้');
                $('#km-type-modal').modal('show');
            }
        });
    });
    ...
    $('#example').on('click', '.btn-delete', function () {
        ...
        if (status) {
            $.ajax({
                url: '<?php echo site_url('delete-km-type'); ?>',
                method: 'post',
                data: {id: uid},
                success: function () {
                    location.reload();
...
<?php $this->load->view("learning_centers/modals/km_type_modal"); ?>