    ...
    function StdEdit(e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#bid").val(data.StdId);
                $("#inStdTitlename2").val(data.std_titlename.trim());
                $("#inStdFirstname2").val(data.std_firstname);
                $("#inStdLastname2").val(data.std_lastname);
                $("#inStdNickname2").val(data.std_nickname);
                $("#inStdIdcard2").val(data.std_idcard);
                $("#inStdReligion2").val(data.std_religion);
                $("#inStdEthnicity2").val(data.std_ethnicity);
                $("#inStdNationality2").val(data.std_nationality);
                $("#inStdCode2").val(data.std_code);

                //std_birthday
                if (data.std_birthday != null) {
                    var dateOfBirth = data.std_birthday.split("-");
                    $("#inStdBirthday2").val(parseInt(dateOfBirth[2]));
                    $("#inStdBirthmonth2").val(parseInt(dateOfBirth[1]));
                    $("#inStdBirthyear2").val(parseInt(dateOfBirth[0]));
                }
                $("#inStdBirthhospital2").val(data.std_birth_hospital);
                $("#inStdBloodtype2").val(data.std_bloodtype);
...
//    }
    ...
    function InsertWaitingStudent(e) {
        $("h3.modal-title").text("ปรับปรุงข้อมูลนักเรียน");
        $("#std-register-insert-modal").modal("show");
    }

    function RoomListOnChange(e) {
//        alert(e.name);
        var status = confirm('ต้องการย้ายห้องเด็ก ?');

        if (status) {
            $.ajax({
                url: '<?php echo site_url('Student/std_registered_move_room'); ?>',
                method: 'post',
                data: {rid: e.value, stdid: e.id, oldid: e.name},
                success: function (data) {
//                    alert(data);
    ...
    function StdMoveOut(e) {
//        var school_class = ;

        var status = confirm('ต้องการย้ายเด็กออกจาก ?');

        if (status) {
            $.ajax({
                url: '<?php echo site_url('Student/std_registered_update'); ?>',
                method: 'post',
                data: {id: e.id, status: 'W'},
                success: function (data) {
                    $.ajax({
