            ...
            <div class="row">
                <form method="post" id="insert-form" class="col-md-9 form-horizontal">
                    <div class="col-md-4 form-group">
                        ...
                        <div class="col-md-6">
                            <input type="number" name="inProvinceStrategicNo" id="inProvinceStrategicNo" class="form-control" autofocus required />
                        </div>
                    </div>
                    <div class="col-md-7 form-group">
                        <label class="control-label col-md-3">ชื่อยุทธศาสตร์</label>
                        <div class="col-md-8">
                            <input type="text" name="inProvinceStrategicName" id="inProvinceStrategicName" class="form-control" required/>
                        </div>
                    </div>
                    <div class="col-md-1">
                ...
                <tbody>
                    ...
                    <?php foreach ($rs as $r): ?>
                        <tr>
                            ...
                            <td style="text-align:center;"><?php echo $r['strategic_no'] ?></td>
                            <td><?php echo $r['strategic_name'] ?>

                                <div class="col-md-12">
                                    <?php
                                    $rst = $this->My_model->get_where_order("tb_province_strategies", array('tb_province_strategic_id' => $r['id']), "strategies_no asc");
                                    foreach ($rst as $itm) {
                                        ?>
                                        <div class="row col-md-12">
                                            กลยุทธ์ที่ <?php echo $itm['strategies_no'] . nbs(2) . $itm['strategies_name'] . nbs(4) ?>
                            ...
                            <?php if ($this->session->userdata("status") == "ผู้ปฏิบัติงาน"): ?>
                                <td style="text-align:center;">
                                    ...
                                    <button type="button" class="col-md-12 btn btn-info btn-strategy" alt='ยุทธศาสตร์ที่ <?php echo $r['strategic_no'] . ' ' . $r['strategic_name'] ?>' id="<?php echo $r['id']; ?>"><i class="icon-plus icon-large"></i> เพิ่มกลยุทธ์</button>
...
//    $("div#example_length.dataTables_length").append("&nbsp;<button class='btn btn-default btn-insert'><i class='icon-print icon-large'></i> พิมพ์ข้อมูล</button>");
    ...
    $("#insert-form").on("submit", function (e) {
        e.preventDefault();
        $.ajax({
            url: "<?php echo site_url('EducationPlan/province_strategic_insert'); ?>",
            method: "post",
    ...
    $("#example").on("click", ".btn-edit", function () {
        ...
        $.ajax({
            url: "<?php echo site_url('EducationPlan/province_strategic_edit'); ?>",
            method: "POST",
            data: {id: uid},
            dataType: "JSON",
            success: function (data) {
                $("#id").val(data.id);
                $("#inProvinceStrategicNo").val(data.strategic_no);
                $("#inProvinceStrategicName").val(data.strategic_name);
            }
        });
    });
    // delete data;
    $("#example").on("click", ".btn-delete", function () {
        ...
        if (status) {
            $.ajax({
                url: "<?php echo site_url('EducationPlan/province_strategic_delete'); ?>",
                method: "post",
                data: {id: uid},
                success: function (data) {
                    location.reload();
    ...
    function EditThisSubRow(e) {
        $.ajax({
            url: "<?php echo site_url('EducationPlan/provice_strategic_definetion_sub_edit'); ?>",
            method: "POST",
            data: {id: e.id},
            dataType: "json",
            success: function (data) {
    ...
    function DeleteThisSubRow(e) {
        ...
        if (status) {
            $.ajax({
                url: "<?php echo site_url('EducationPlan/provice_strategic_definetion_sub_delete'); ?>",
                method: "post",
                data: {id: e.id},
                success: function (data) {
                    location.reload();
...
<?php $this->load->view("education_plan/modals/province_strategies_modal"); ?>